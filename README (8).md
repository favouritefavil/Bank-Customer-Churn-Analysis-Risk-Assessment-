...
**Source:** Simulated Nigerian commercial bank customer data  
...
### Page 1 — Portfolio Overview
![Portfolio Overview](dashboard/portfolio_overview.png)

---

### Page 2 — Engagement & Product Depth
![Engagement & Product Depth](dashboard/engagement_product_depth.png)

---

### Page 3 — Risk & Geography
![Risk & Geography](dashboard/risk_and_geography.png)
...
This analysis is based on one portfolio snapshot, not a time series. Churn trends, engagement score movements, and balance changes over time — all of which would strengthen the analysis — are not available in this dataset.

**2. Churn flags are outcome variables, not predictive scores.**  
`churn_30d` and `churn_90d` record whether a customer already churned, not how likely they are to churn in the future. The risk signals used in this analysis (low engagement, inactivity) are proxies for future churn risk, not modelled predictions.

**3. No transaction-level detail.**  
The dataset contains transaction counts and volumes for the last 30 days but not individual transaction records. This limits the ability to understand what customers are actually doing — bill payments, transfers, withdrawals — and which transaction types correlate most strongly with retention.

**4. Self-reported or system-generated income data.**  
`monthly_income_ngn` may reflect self-reported figures or bank estimates rather than verified income. Segment analysis based on income tiers should be interpreted with this caveat.

**5. Simulated dataset.**  
The data was generated to simulate a Nigerian commercial bank portfolio. Patterns and relationships are designed to be realistic, but findings should be validated against real customer data before driving operational decisions.

**6. No competitor or external data.**  
Churn reasons are inferred from behavioural signals (engagement, inactivity, product depth) rather than direct exit surveys. Some churn may be driven by competitor pricing or product offerings that are not visible in this dataset.
...
*Dataset: Simulated Nigerian Commercial Bank customer data*
